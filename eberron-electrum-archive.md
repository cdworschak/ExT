...
**One Maneuver per action, no stacking — a rule that was genuinely never stated anywhere until now, despite the whole table implicitly depending on it.** Each Maneuver either *is* your action for the turn (Guard Stance, Taunt, Rally, Read the Fight — anything that doesn't require also swinging a weapon) or *modifies* your one attack this turn (Focus, Cleave, Guarded Strike). Since a turn only grants one attack under normal action economy, **only one attack-modifying Maneuver can ever apply to a single attack** — Focus and Cleave can't both land on the same swing, no matter how much Grit is available. Maneuvers combine across different PCs' turns, or across different rounds for the same PC, never by stacking two modifiers onto one action.
...
**Final, trustworthy result:** zero-death rate 92.5% (sensibly above the established 76.5–82.0% band, since an all-frontline party is naturally tougher than the mixed-archetype squad that band was calibrated against). Individual down rates: Rurik (Heavy 2H) 2.8%, Sable (Versa+Shield) 0.6%, Toran (Versa+Tower) 0.5%, Kade (Dual-wield) 1.9%, Fen (Unarmed) 1.9% — **a tight, correctly-ordered band matching each loadout's actual Guard value, no outliers, no dominant strategy.** Closes out the Dual-wield/Shield/Two-handed redesign with genuine full-party evidence, not just 1-on-1 math.
...
- **Shard-Split** — shard cost (as the spell it's attached to). A single-target cast also strikes one other creature in the same zone at half effect, no extra shard cost beyond the base cast. **If no second creature is present, the cast instead gets Advantage** — corrected from an earlier version with no fallback at all, a genuine conditional trap: real value against groups, completely wasted (same cost, zero benefit) against any solo target, including most bosses. Now never strictly worse than a normal cast regardless of what's actually in the zone.
...
- **Zealous Rebuke** — once per scene, no shard cost. When an ally is hit, mark the attacker: your next hit against that specific enemy deals +1d6. Corrected from an earlier "always-on, no gate" version — genuinely undercosted for a free passive, since the bonus matched a whole 2-shard Shard Bolt cast for zero cost, unlimited uses. Capped at once per scene rather than reducing the bonus itself, preserving the payoff while removing the unlimited overreach — same limiter pattern every other Tradition Option already uses.
...
- **Thorned Ground** — once per scene. For the rest of the encounter, any enemy that leaves your zone automatically takes 1d4 damage, no check, no roll to avoid — their choice to make. Built from a flat automatic-damage mechanic rather than a "difficult terrain" status this system has no support for (zones don't have granular movement distance to slow). **Genuinely narrower trigger than its sibling Fractal Ward** (enemies must actually retreat, which many fights never call for, versus Fractal Ward's much more common "you got hit") — considered for a buff, but accepted as-is: a niche, situational tool doesn't need to fire as often as a universal one to be worth the pick, the same way Undying Witness's rare trigger isn't treated as a flaw. No mechanical change made.
- **Bloodscent** — Spark-tier shard cost. Mark a Wounded enemy: you and allies gain Advantage against that specific target until it's down. **Target-dependent value, same caveat Feint already carries** — Advantage barely moves the needle against a target the party can already hit reliably (tested at only +0.3 percentage points in one check at typical stat levels vs. a Standard-tier target), and reads as more consistently useful than it actually is. Genuinely strong against a tough, hard-to-hit target; close to free against an easy one.
...
**Aberrant Mark** — grants a Minor-tier cast attempt with zero shards ever possessed, meaning every use is a genuine Overcast, resolved entirely through the existing Overcast Backlash Pool (always 3d6, since 0 shards is always more than half short). No separate mechanic — reuses casting and Overcasting directly. Was flagged for a long stretch as never built out; now resolved.

**Advantage / Disadvantage** — roll 3d10 instead of 2d10, keep the best two (Advantage) or worst two (Disadvantage). Disadvantage rolls never crit. Never stacks *within a single roll* — that roll has one, the other, or neither; both at once on the same roll cancels to normal. Strictly per-roll, not per-character: a PC's own outgoing attack and an enemy's incoming attack against that PC are separate rolls that never interact, even in the same round.
...
- ~~A trap-option audit of the Tradition-Specific Options found several real issues~~ — **all four resolved.** Shard-Split: fixed to grant Advantage when no second target exists, never wasted regardless of what's in the zone. Zealous Rebuke: capped at once per scene, fixing the undercosted-for-free issue without touching the actual bonus. Bloodscent: given the same target-dependent-value caveat Feint already carries — genuinely strong against tough targets, close to free against easy ones. Thorned Ground: considered for a buff, accepted as-is — a niche tool doesn't need to fire as often as a universal one to earn its pick.

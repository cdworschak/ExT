...
- **`docs/`** — a complete, static HTML rulebook built from the master document, stripped of design notes — just the finished rules. Eleven pages, ordered around Character Creation's own flow: Home, Foundations, Character Creation, Backgrounds, Archetypes, Magic, Combat, Exploration, Enemy Design, Advancement, Glossary. No build step — open `docs/index.html` directly, or serve the folder via GitHub Pages. Named `docs` specifically because GitHub Pages' "Deploy from a branch" option only supports the repo root or a folder literally named `docs`. Contains a `.nojekyll` file, which tells GitHub Pages to skip its default Jekyll build and serve the plain HTML as-is.
...
- **`tools/CharacterCreator.jsx`** — a browser-based character creation wizard (React). Needs a build step or a Babel-standalone wrapper to run outside Claude's artifact environment.
